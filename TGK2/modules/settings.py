...
# Apikey
PROXY6NET_API_KEY: str = os.getenv('PROXY6NET_API_KEY')

# Apikey
OPENAI_API_KEY: str = os.getenv('OPENAI_API_KEY')

# Time zone ( Should be the same as in django )
TZ = ZoneInfo("Europe/Moscow")

# Logger time zone
LOGGER_TZ = pytz.timezone("Europe/Moscow")

# Default comment if ai shits
DEFAULT_COMMENT: str = '🔥🔥🔥'

# Min len of post to sending comment
POST_MIN_LEN: int = 140

# Delete this symbols from ai answer for comment text
SYMBOLS_TO_DELETE: list[str] = ['«', '»', '>', '<', '"', '«', '»', '《', '》', '(', ')', '[', ']', '{', '}', '„', '“', '❝', '❞']

# Replace this symbols from ai answer for comment text (Left - old, Right - new)
SYMBOLS_TO_REPLACE: list[tuple[str]] = [
    ('—', '-'),
    ('…', '...'),
    ('−', '-'),
    ('°', 'градус'),
...
# Range to random active in first days
DAYS_ACTIVE: dict = {
    "0": [1, 1],
    "1": [1, 2],
    "2": [1, 5],
...
COMMENT_PROMT_1: str = """
Проанализируй пост, улови его смысл, настроение и острые моменты. 
Представь, что ты: {}, 22 года, с хорошим чувством юмора и умением писать живые комменты. 

Сгенерируй 5–7 комментариев, которые: 
1. Лёгкие, цепкие и живые – чтобы люди захотели ответить или хотя бы улыбнулись. 
2. Разные по стилю: 
- Смешное возмущение
- Простой сарказм
- Личный опыт, но без драмы
- Шутка на грани абсурда
- Неожиданный поворот
3. Разговорные, без занудства – как будто просто пишешь другу.
4. Без банальных фраз – никаких «Интересно, а что вы думаете?».
5. Минимум букв, максимум смысла – коротко, но метко. 
Формат ответа: выбери один лучший комментарий и выведи ТОЛЬКО его текст, без пояснений и предисловий. 

Пост: {} 
"""

COMMENT_PROMT_2: str = """
- Сожми комментарий до одного цепляющего предложения.
- Если уместно: используй эмодзи.
- Если уместно: используй улыбку ")" (закрывающую скобку).
Формат ответа: выведи ТОЛЬКО текст сжатого комментария, без пояснений и предисловий.
...
REPLY_PROMT_1: str = """
Проанализируй пост, улови его смысл, настроение и острые моменты.
Представь, что тебе 22 года, ты с хорошим чувством юмора и умением писать живые комменты.

Теперь представь, что ты уже оставил комментарий под этим постом, и тебе кто-то ответил.

Сгенерируй 5–7 ответов на этот ответ, которые:
1. Лёгкие, цепкие и живые – чтобы человек захотел продолжить переписку или хотя бы улыбнулся.
2. Разные по стилю:
- Смешное возмущение
- Простой сарказм
- Личный опыт, но без драмы
- Шутка на грани абсурда
- Неожиданный поворот
3. Разговорные, как будто пишешь знакомому в комментах.
4. Без банальных фраз вроде «Согласен», «Интересно мнение» и прочего шаблона.
5. Минимум букв, максимум смысла – коротко, но метко.

Формат ответа: выбери один лучший вариант и выведи ТОЛЬКО его текст, без пояснений и предисловий.

Пост: {}
Мой комментарий: {}
Ответ пользователя на мой комментарий: {}
"""

REPLY_PROMT_2: str = """
- Сожми ответ до одного цепляющего предложения.
- Если уместно: используй эмодзи.
- Если уместно: используй улыбку ")" (закрывающую скобку).
Формат ответа: выведи ТОЛЬКО текст сжатого ответа, без пояснений и предисловий.
