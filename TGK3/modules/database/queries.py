        ...
        async with session.begin():
            ...
            for account in accounts:
                accounts_list.append({
                    ...
                    "need_update": account.need_update
                })
...
async def deactivate_order_by_order_id(order_id: int):
    async with async_session() as session:
...
async def increment_completed_comment_posts(order_id: int):
    async with async_session() as session:
...
async def increment_completed_ad_days(order_id: int):
    async with async_session() as session:
        stmt = select(Order).filter(Order.id == order_id)
        result = await session.execute(stmt)
        order = result.scalars().first()
        ...
        order.completed_ad_days = int(order.completed_ad_days) + 1
        await session.commit()
...
async def set_need_update(account_id: int, value: bool):
    async with async_session() as session:
